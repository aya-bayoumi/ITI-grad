<template>
   <div class="px-20 my-10 flex justify-between items-center">
     ...
     <div>
       <p class="font-normal text-2xl">books <span class="font-medium">>book details</span></p>
       <img class="w-50 h-70 mt-5" :src="book.image  ||'/images/basic.png'" @error="e => e.target.src = '/images/basic.png'" alt="">

     </div>
     <div class="w-[900px] self-stretch">
       <h1 class="text-3xl font-semibold font-['Playfair_Display'] mb-3">{{ book.title }}</h1>
       <p class="font-['Montserrat'] text-xl font-semibold mb-3" v-if="book && book.author && book.author.name">{{ book.author.name }}</p>
